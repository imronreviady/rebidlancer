<?php
	$single_freelancer_info = $this->db->get_where('freelancer', array('freelancer_id' => $param2))->result_array();
...
<div class="user-box">
    <div class="img-relative">
		...
		<!-- Hidden upload form -->
		<form method="post" action="<?= base_url(); ?>admin/freelancers/change_profile_image/<?= $row['freelancer_id'] ?>" enctype="multipart/form-data" id="picUploadForm" target="uploadTarget">
		...
		<!-- Profile image -->
		<img src="<?= $this->core_model->get_image_url('freelancer', $row['freelancer_id']);?>" class="img-circle" id="imagePreview">
    </div>
</div>
...
<form role="form" action="<?= base_url(); ?>admin/freelancers/update/<?= $row['freelancer_id'] ?>" method="post" enctype="multipart/form-data">
